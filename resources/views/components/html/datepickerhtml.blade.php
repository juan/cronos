...
<div
 ...
 >
    ...
    <input  wire:model="{{$id}}"
            type="text" id="{{$id}}" name="{{$id}}" {{ $attributes }}
           class="block px-1.5 pb-1.5 pt-2.5 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1
...
<script>
    document.addEventListener("DOMContentLoaded", function(){
        let element = document.getElementById("{{$id}}"); //id of the input element
        let options = {autohide: true, format: 'dd/mm/yyyy', language: 'es',  todayHighlight: true}
        let datepicker = new Datepicker(element,options);
        element.addEventListener('changeDate', function (evt) {

            @this.set('{{$id}}', document.getElementById("{{$id}}").value,true)
        });
    }
